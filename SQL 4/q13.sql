...
SELECT ID, SCORE,
    CUME_DIST() OVER (ORDER BY SCORE DESC) AS CUMULATIVE_PERCENTAGE
FROM STUDENT
ORDER BY ID;
...
SELECT
    ID,
    SCORE,
    CUMULATIVE_PERCENTAGE -- 누적 백분율 컬럼 추가
FROM (
    SELECT
        ID,
        SCORE,
...
WHERE
    CUMULATIVE_PERCENTAGE > 0.3
    AND CUMULATIVE_PERCENTAGE <= 0.7
ORDER BY
    CUMULATIVE_PERCENTAGE ASC; -- 누적 백분율 기준으로 오름차순 정렬